...
public class GiangVienDAO extends AbstractDAO<GiangVien>{
  ...
  private final String UPDATE_SQL="update giangvien set hoten=?,gioitinh=?,diachi=?,ngaysinh=?,sdt=? where magv=?";
  ...
  public boolean update(GiangVien dto) throws SQLException {
    try(PreparedStatement stm=con.prepareStatement(UPDATE_SQL)){
      ...
      stm.setBoolean(2, dto.isGioitinh());
      stm.setString(3, dto.getDiachi());
      stm.setDate(4, dto.getNgaysinh());
      stm.setString(5, dto.getSdt());
      stm.setString(6, dto.getMagv());
